...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add FAQ </a>
        </div>
    </div>
                         ...
                         <?php  }  ?>
                            <table id="nhealth-table" class="table table-striped table-bordered table-hover" width="100%"
                                ...
                                <thead>
                                    ...
                                    <tr class="stdfilters">
                                        <th><input type="text" placeholder="Question"></th>
                                        <th id="drop-searc"></th>
                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle" id="statusToggle<?php echo $faq['faq_id']; ?>" <?php echo ($faq['publish']) ? 'checked' : ''; ?> data-status-id="<?php echo $faq['faq_id']; ?>">
                                                    <label class="custom-control-label" for="statusToggle<?php echo $faq['faq_id']; ?>">
                                                        <?php echo ($faq['publish']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                        </td>
                                        <td class="text-right">
                                            <a style="display: inline-block;" href="<?php echo $faq['edit']; ?>"
                                                data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                            <form style="display: inline-block;"
                                                action="<?php echo $faq['delete']; ?>" method="post"
                                                enctype="multipart/form-data"
                                                id="del_faq<?php echo $faq['faq_id']; ?>">
                                                <input type="hidden" name="faq_id"
                                                    value="<?php echo $faq['faq_id']; ?>">
                                                <button type="button" data-toggle="tooltip"
                                                    title="<?php echo $button_delete; ?>" class="btn btn-danger"
                                                    onClick='submitDeleteForm("del_faq<?php echo $faq['faq_id']; ?>")'><i
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        $.ajax({
            url: '<?php echo $ajaxupdatefaqsstatus; ?>',
            method: 'POST',
            data,
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    if (response.success) {
                        ...
                        rowData[1] = '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input status-toggle" id="statusToggle' + faqId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' + faqId + '"> <label class="custom-control-label" for="statusToggle' + faqId + '">' + newStatusText + '</label> </div>';
