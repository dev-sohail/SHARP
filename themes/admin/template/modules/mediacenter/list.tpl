...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Media Center </a>
        </div>
    </div>
                    ...
                    <div class="card">
                        <div class="card-bodys">
                            <table class="table table-striped table-bordered table-hover" id="investment-table" width="100%" cellspacing="0" cellpadding="0" border="0">
                                ...
                                <tbody>
                                    <?php if ($media_centers) { ?>
                                        <?php foreach ($media_centers as $media_center) { ?>
                                            <tr class="stdfilters">
                                                <td class="text-left">
                                                    <?php echo $media_center['title']; ?>
                                                </td>
                                                <td class="text-left">
                                                    <?php echo $media_center['sort_order']; ?>
                                                </td>

                                                <td class="text-left">
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $media_center['media_center_id']; ?>"
                                                        <?php echo ($media_center['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $media_center['media_center_id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $media_center['media_center_id']; ?>">
                                                        <?php echo ($media_center['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                                <td class="text-right">
                                                    <a style="display: inline-block;" href="<?php echo $media_center['edit']; ?>" data-toggle="tooltip" title="<?php echo $button_edit; ?>" class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                    <form style="display: inline-block;" action="<?php echo $media_center['delete']; ?>" method="post" enctype="multipart/form-data" id="del_media_center<?php echo $media_center['media_center_id']; ?>">
                                                        <input type="hidden" name="media_center_id" value="<?php echo $media_center['media_center_id']; ?>">
                                                        <button type="button" data-toggle="tooltip" title="<?php echo $button_delete; ?>" class="btn btn-danger" onClick='submitDeleteForm("del_media_center<?php echo $media_center['media_center_id']; ?>")'><i class="fa fa-trash-o"></i></button>
                                                    </form>
                                                </td>
                                            </tr>
                                        <?php } ?>
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        $('#loader').show();
        let data = {
            media_center_id: mediaId,
            status: newStatus
        };
        $.ajax({
            url: '<?php echo $ajaxmediacenterstatus; ?>',
            method: 'POST',
            data,
