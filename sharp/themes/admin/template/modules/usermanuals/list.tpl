...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add User Manual </a>
        </div>
    </div>
                                            ...
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $manual['manual_id']; ?>"
                                                        <?php echo ($manual['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $manual['manual_id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $manual['manual_id']; ?>">
                                                        <?php echo ($manual['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                            <td class="text-right">
                                                <a style="display: inline-block;" href="<?php echo $manual['edit']; ?>"
                                                    data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                    class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                <form style="display: inline-block;"
                                                    action="<?php echo $manual['delete']; ?>" method="post"
                                                    enctype="multipart/form-data"
                                                    id="del_manual<?php echo $manual['manual_id']; ?>">
                                                    <input type="hidden" name="manual_id"
                                                        value="<?php echo $manual['manual_id']; ?>">
                                                    <button type="button" data-toggle="tooltip"
                                                        title="<?php echo $button_delete; ?>" class="btn btn-danger"
                                                        onClick='submitDeleteForm("del_manual<?php echo $manual['manual_id']; ?>")'><i
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var ManualId = checkbox.data('status-id');
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    rowData[1] =
                        ...
                        ManualId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' +
                        ManualId + '"> <label class="custom-control-label" for="statusToggle' +
                        ManualId + '">' + newStatusText + '</label> </div>';
