...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Home Slider </a>
        </div>
    </div>
                                            ...
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $slider['slider_id']; ?>"
                                                        <?php echo ($slider['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $slider['slider_id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $slider['slider_id']; ?>">
                                                        <?php echo ($slider['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                            <td class="text-right">
                                                <a style="display: inline-block;" href="<?php echo $slider['edit']; ?>"
                                                    data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                    class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                <form
                                                    action="<?php echo $slider['delete']; ?>" method="post"
                                                    enctype="multipart/form-data"
                                                    id="del_slider<?php echo $slider['slider_id']; ?>">
                                                    <input type="hidden" name="slider_id"
                                                        value="<?php echo $slider['slider_id']; ?>">
                                                    <button type="button" data-toggle="tooltip"
                                                        title="<?php echo $button_delete; ?>" class="btn btn-danger"
                                                        onClick='submitDeleteForm("del_slider<?php echo $slider['slider_id']; ?>")'><i
...
<script type="text/javascript">
    // $('#slider-table thead tr').clone(true).addClass('stdfilters').appendTo('#slider-table thead');
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var sliderId = checkbox.data('status-id');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    //     '</div>'
                    // );
                    var row = $('#slider-table').DataTable().row(checkbox.closest('tr'));
                    var rowData = row.data();
                    rowData[1] =
                        '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input status-toggle" id="statusToggle' +
                        sliderId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' +
                        sliderId + '"> <label class="custom-control-label" for="statusToggle' +
                        sliderId + '">' + newStatusText + '</label> </div>';
