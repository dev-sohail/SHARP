...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Brand </a>
        </div>
    </div>
                                    ...
                                    <tr class="stdfilters">
                                        ...
                                        <div class="custom-control custom-switch">
                                            <input type="checkbox" class="custom-control-input status-toggle"
                                                id="statusToggle<?php echo $brand['brand_id']; ?>"
                                                <?php echo ($brand['status']) ? 'checked' : ''; ?>
                                                data-status-id="<?php echo $brand['brand_id']; ?>">
                                            <label class="custom-control-label"
                                                for="statusToggle<?php echo $brand['brand_id']; ?>">
                                                <?php echo ($brand['status']) ? 'Active' : 'Inactive'; ?>
                                            </label>
                                        </div>
                                        </td>
                                        <td class="text-right">
                                            <a style="display: inline-block;" href="<?php echo $brand['edit']; ?>"
                                                data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                              <form style="display: inline-block;"
                                                action="<?php echo $brand['delete']; ?>" method="post"
                                                enctype="multipart/form-data"
                                                id="del_team<?php echo $brand['brand_id']; ?>">
                                                <input type="hidden" name="brand_id"
                                                    value="<?php echo $brand['brand_id']; ?>">
                                                <button type="button" data-toggle="tooltip"
                                                    title="<?php echo $button_delete; ?>" class="btn btn-danger"
                                                    onClick='submitDeleteForm("del_team<?php echo $brand['brand_id']; ?>")'><i class="fa fa-trash-o"></i></button>
                                            </form>
                                        </td>
                                       </tr>
                                    <?php } ?>
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var brandId = checkbox.data('status-id');
        ...
        $.ajax({
            url: '<?php echo $ajaxbrandssstatus; ?>',
            method: 'POST',
            data,
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    rowData[3] =
                        ...
                        brandId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' +
                        brandId + '"> <label class="custom-control-label" for="statusToggle' +
                        brandId + '">' + newStatusText + '</label> </div>';
